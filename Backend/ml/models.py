...
class Service_info(models.Model):
    ...
    description = models.TextField(max_length=300,blank=True,default='')
    mod = models.ForeignKey(to=Model_info, on_delete=models.CASCADE, null=True) # 对应的模型

    create_time = models.DateTimeField(auto_now_add=True)
    recent_modified_time = models.DateTimeField(auto_now_add=True)
    status = models.IntegerField(blank=True,null=True,default=0) #0:Deployed, 1:pause
    # 调用次数，平均使用时间，mod
...
class Test_info(models.Model):
    ...
    service = models.ForeignKey(to=Service_info, on_delete=models.CASCADE,blank = True,null=True)